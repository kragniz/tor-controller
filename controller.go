...
const (
	...
	// ErrResourceExists is used as part of the Event 'reason' when a OnionService fails
	// to sync due to a Deployment of the same name already existing.
	ErrResourceExists = "ErrResourceExists"

	// MessageResourceExists is the message used for Events when a resource
	// fails to sync due to a Deployment already existing
	MessageResourceExists = "Resource %q already exists and is not managed by OnionService"
	// MessageResourceSynced is the message used for an Event fired when a OnionService
	// is synced successfully
	MessageResourceSynced = "OnionService synced successfully"
)

// Controller is the controller implementation for OnionService resources
...
func (c *Controller) processNextWorkItem() bool {
	...
	err := func(obj interface{}) error {
		...
		// OnionService resource to be synced.
...
// converge the two. It then updates the Status block of the OnionService resource
...
func (c *Controller) syncHandler(key string) error {
	...
	// Get the OnionService resource with this namespace/name
	onionService, err := c.onionServicesLister.OnionServices(namespace).Get(name)
	if err != nil {
		// The OnionService resource may no longer exist, in which case we stop
		// processing.
		if errors.IsNotFound(err) {
			runtime.HandleError(fmt.Errorf("onionService '%s' in work queue no longer exists", key))
			return nil
		}
	...
	c.recorder.Event(onionService, corev1.EventTypeNormal, SuccessSynced, MessageResourceSynced)
	return nil
}
...
// enqueueOnionService takes a OnionService resource and converts it into a namespace/name
// string which is then put onto the work queue. This method should *not* be
// passed resources of any type other than OnionService.
func (c *Controller) enqueueOnionService(obj interface{}) {
	var key string
	var err error
...
// to find the OnionService resource that 'owns' it. It does this by looking at the
// objects metadata.ownerReferences field for an appropriate OwnerReference.
// It then enqueues that OnionService resource to be processed. If the object does not
